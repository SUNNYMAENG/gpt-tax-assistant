    ...
    try {
        ...
        if (jsonMatch) {
            try {
                ...
                console.log('✅ GPT 조건:', condition); // GPT가 보낸 조건 확인
                ...
                console.warn('⚠️ JSON 파싱 실패:', err.message, '원본:', jsonMatch[0]);
            }
        }
...
app.post('/generate', (req, res) => {
    const data = req.body;
    console.log('➡️ /generate 데이터:', data); // /generate로 넘어온 데이터 확인
    ...
    // ⚠️ 실제 일본 세법에 따른 계산 로직 필요
    const health = data.hasHealth ? Math.round(gross * 0.0984) : 0; // 예시 비율, 실제와 다를 수 있음
    const pension = data.hasPension ? Math.round(gross * 0.0915) : 0; // 예시 비율, 실제와 다를 수 있음
    const empIns = data.hasEmpIns ? Math.round(gross * 0.006) : 0;   // 예시 비율, 실제와 다를 수 있음

    const tax = Math.round(gross * 0.05);       // 매우 단순화된 소득세 예시
    const residentTax = Math.round(gross * 0.10); // 매우 단순화된 주민세 예시
